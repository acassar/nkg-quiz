...
/** Stores the player's answer per question (questionId → choiceId). Source of truth is the backend. */
...
export function useSessionState() {
  ...
  function setPlayerAnswers(answers: Record<string, number>) {
    playerAnswers.clear();
    for (const [qId, cId] of Object.entries(answers)) {
      playerAnswers.set(parseInt(qId), cId);
    }
  }
